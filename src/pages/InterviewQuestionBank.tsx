...
const POPULAR_COMPANIES = [
  'Google', 'Amazon', 'Microsoft', 'Apple', 'Meta', 'Netflix', 'Salesforce', 'Adobe',
  'Samsung', 'Tata', 'Infosys', 'SAP', 'Alibaba', 'Toyota', 'Siemens', 'Accenture',
  'Deloitte', 'McKinsey', 'JPMorgan', 'Goldman Sachs', 'HSBC', 'Unilever',
];
...
const InterviewQuestionBank = () => {
  ...
  const handleUpvote = async (questionId: string) => {
    ...
    try {
      await supabase.from('interview_questions' as any).update({ upvotes: questions.find(q => q.id === questionId)!.upvotes + 1 }).eq('id', questionId);
    } catch { /* best effort */ }
  };
  ...
  const generateTemplateQuestions = (company: string, role: string): InterviewQuestion[] => {
    ...
    const templates: Record<string, { q: string; tips: string[]; answer: string }[]> = {
      ...
      technical: [
        { q: `How would you design a scalable notification system for ${company}?`, tips: ['Discuss message queues', 'Consider rate limiting', 'Think about delivery guarantees'], answer: `I'd use a pub/sub model with a message queue (Kafka/SQS), a priority system for different notification types, fan-out for multiple channels, and a dead-letter queue for failed deliveries.` },
        { q: `What's your approach to debugging a production performance issue?`, tips: ['Mention monitoring tools', 'Discuss systematic approach', 'Talk about rollback strategies'], answer: `Start with metrics, identify the regression timeframe, correlate with recent deploys, use profiling tools to isolate bottlenecks, apply a fix with a feature flag, and add monitoring to prevent recurrence.` },
      ],
      'system-design': [
        { q: `Design a real-time feed system similar to what ${company} might use.`, tips: ['Start with requirements', 'Discuss data model', 'Address scaling challenges'], answer: `Requirements: low latency, ordered feed, personalized ranking. Architecture: write path (event → queue → fanout), read path (cache-first with fallback), ranking service, and a hybrid push/pull model.` },
      ],
      'case-study': [
      ...
      'culture-fit': [
        { q: `Why do you want to work at ${company} specifically?`, tips: ['Show genuine knowledge', 'Connect to your values', 'Mention specific products'], answer: `I admire ${company}'s commitment to [specific value]. My experience aligns with your mission. I'm particularly excited about [recent initiative].` },
        { q: `How do you handle receiving critical feedback?`, tips: ['Show growth mindset', 'Give a real example', 'Demonstrate self-awareness'], answer: `I view feedback as a growth opportunity. Recently, I received feedback about my communication style. I started summarizing action items after each meeting, which improved team alignment significantly.` },
      ],
    };
    ...
    for (const cat of categories) {
      for (const tq of (templates[cat] || [])) {
        id++;
        result.push({ id: `q-${id}`, company, role, question: tq.q, category: cat, difficulty: cat === 'system-design' ? 'hard' : cat === 'technical' ? 'medium' : 'easy', sampleAnswer: tq.answer, tips: tq.tips, upvotes: Math.floor(10 + Math.random() * 200), source: 'ai' });
      }
    }
    return result;
  };
  ...
  const getDifficultyColor = (d: string) => d === 'easy' ? 'bg-green-500/10 text-green-600' : d === 'medium' ? 'bg-amber-500/10 text-amber-600' : 'bg-red-500/10 text-red-600';
  const getCategoryIcon = (cat: string) => ({ behavioral: '🧠', technical: '💻', 'system-design': '🏗️', 'case-study': '📊', 'culture-fit': '🤝' }[cat] || '❓');
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground flex items-center gap-3"><BookOpen className="w-8 h-8 text-primary" />Interview Question Bank</h1>
          <p className="text-muted-foreground mt-1">AI-generated questions specific to each company — cached for instant access</p>
        </div>
          ...
          <CardContent className="pt-6">
            <div className="flex flex-wrap gap-3 items-end">
              <div className="flex-1 min-w-[200px]">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Enter any company name..." value={searchCompany} onChange={e => setSearchCompany(e.target.value)} className="pl-9" onKeyDown={e => e.key === 'Enter' && generateQuestions(searchCompany)} />
                </div>
              </div>
              <div className="w-56">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              {POPULAR_COMPANIES.map(c => (
                <Badge key={c} variant="outline" className="cursor-pointer hover:bg-primary/10 text-xs" onClick={() => { setSearchCompany(c); generateQuestions(c); }}>{c}</Badge>
              ))}
            </div>
          </CardContent>
        </Card>
                ...
                <Card key={q.id}>
                  <CardContent className="pt-6">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Button variant="ghost" size="sm" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary" onClick={() => handleUpvote(q.id)}>
                        <ThumbsUp className="w-3 h-3" /> {q.upvotes}
                      </Button>
                    </div>
                    <h3 className="text-base font-semibold mb-3">{q.question}</h3>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {q.tips.map((tip, i) => (<Badge key={i} variant="outline" className="text-xs font-normal">💡 {tip}</Badge>))}
                    </div>
                    {q.sampleAnswer && (
                      <Accordion type="single" collapsible>
                        <AccordionItem value="answer" className="border-none">
                          ...
                          <AccordionContent>
                            <div className="bg-muted/50 rounded-lg p-4 text-sm leading-relaxed">{q.sampleAnswer}</div>
                          </AccordionContent>
                        </AccordionItem>
                      </Accordion>
                    )}
